...
def export_transcript(
    history_store: Any, session_id: str, target: str | None = None
) -> Path:
    ...
    destination = (
        Path(target).expanduser() if target else _default_export_path(session_id)
    )
...
def _switch_session(config: Any, history_store: Any, session_id: str) -> Any:
...
def _print_chats(history_store: Any, session_id: str) -> None:
...
def main() -> None:
    ...
    print(
        "Commands: /chats, /switch <id>, /rename <title>, /close, /reset, /export [path], exit"
    )
    ...
    while True:
        ...
        if lowered == "/chats":
            _print_chats(history_store, current_session_id)
            continue
        ...
        if lowered.startswith("/switch "):
            ...
            session = _switch_session(config, history_store, current_session_id)
        ...
        if lowered.startswith("/export"):
            ...
            try:
                exported = export_transcript(
                    history_store, current_session_id, raw_target or None
                )
        ...
        try:
            ...
            print(
                "Connection error reaching OpenAI API. Check internet/VPN/firewall and try again."
            )
            continue
        ...
        history_store.append_message(
            session_id=current_session_id, role="user", content=user_input
        )
        history_store.append_message(
            session_id=current_session_id,
            role="assistant",
