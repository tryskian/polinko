...
echo "5. Existing repeated-run OCR baseline"
"$python_bin" - <<'PY'
from __future__ import annotations
...
echo "6. Retained manual feedback cohorts"
"$make_bin" --no-print-directory manual-evals-feedback-cohorts
echo
...
the mature lane. The manual feedback cohorts show that retained failures remain
visible without mutating the warehouse during the demo.
""",
	encoding="utf-8",
)
