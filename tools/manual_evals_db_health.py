...
def _feedback_source_context_mutation_boundary() -> dict[str, str]:
    return {
...
def _feedback_source_context_blocker(code: str, detail: str) -> dict[str, str]:
    return {"code": code, "detail": detail}


def _feedback_source_context_fingerprint(source_context: dict[str, Any]) -> str:
...
def _source_context_message_packet(
    ...
    *,
    ...
    if not source_history_value:
        return (
            ...
            [
                _feedback_source_context_blocker(
    ...
    messages = [
        *[
            _source_context_message_packet(item, position="before")
            for item in reversed(before_rows)
        ],
        _source_context_message_packet(target, position="target"),
        *[
            _source_context_message_packet(item, position="after")
...
def _feedback_source_context_item(row: dict[str, Any]) -> dict[str, Any]:
    source_context, blockers = _source_feedback_message_context(row)
    source_context["fingerprint"] = _feedback_source_context_fingerprint(source_context)
    action_cohort = row.get("action_cohort")
    if not isinstance(action_cohort, dict):
        action_cohort = {}
    return {
...
def build_feedback_source_context_report(
    *,
    ...
    outcome_filter = _normalize_outcome_filter(outcome) or "fail"
    cohort_filter = _normalize_cohort_filter(cohort) or "grounding_source_verification"
    ...
    if not db_path.is_file():
        return {
            ...
            "mutation_boundary": _feedback_source_context_mutation_boundary(),
            "items": [],
            "blockers": [
                _feedback_source_context_blocker(
    ...
    items = [_feedback_source_context_item(row) for row in selected_rows]
    item_blockers = [
        blocker
        for item in items
...
def _overlay_comparison_mutation_boundary() -> dict[str, str]:
    return {
        "manual_evals_db": "read_only",
        "source_history_db": "read_only",
        "feedback_status": "unchanged",
...
def _overlay_comparison_blocker(code: str, detail: str) -> dict[str, str]:
    return {"code": code, "detail": detail}


def _overlay_source_index_blocker(code: str, detail: str) -> dict[str, str]:
    return {"code": code, "detail": detail}


def _load_overlay_source_context_index(
...
def _source_index_public(source_index: dict[str, Any]) -> dict[str, Any]:
...
def _overlay_source_image_candidate(evidence_row: dict[str, Any]) -> dict[str, Any]:
...
def _overlay_index_source_images(
    *,
    source_item: dict[str, Any],
    source_context: dict[str, Any],
...
def _overlay_comparison_payload_preview(
    *,
    source_item: dict[str, Any],
    source_context: dict[str, Any],
...
def _overlay_comparison_item(
    ...
    *,
    ...
    if not isinstance(action_cohort, dict):
        action_cohort = {}
    ocr_source_images = [_overlay_source_image_candidate(row) for row in evidence_rows]
    indexed_source_images, index_blockers = _overlay_index_source_images(
        source_item=source_item,
        source_context=source_context,
    ...
    payload_preview = _overlay_comparison_payload_preview(
        source_item=source_item,
        source_context=source_context,
...
def build_overlay_ocr_comparison_readiness_report(
    *,
    ...
    outcome_filter = _normalize_outcome_filter(outcome) or "fail"
    cohort_filter = _normalize_cohort_filter(cohort) or "ocr_overlay_hypothesis"
    row_limit = max(1, limit)
    source_index, source_index_entries = _load_overlay_source_context_index(
        overlay_source_index_path
    )
    public_source_index = _source_index_public(source_index)
    ...
    return {
        ...
        "mutation_boundary": _overlay_comparison_mutation_boundary(),
        "items": items,
...
def _feedback_decision_preview_blocker(code: str, detail: str) -> dict[str, str]:
    return {"code": code, "detail": detail}


def _load_feedback_decision_payload(
    decision_path: Path,
) -> tuple[dict[str, Any] | None, list[dict[str, str]]]:
    resolved_decision_path = decision_path.expanduser()
    if not resolved_decision_path.is_file():
        return None, [
            _feedback_decision_preview_blocker(
...
def _feedback_decision_would_apply(
    *,
    ...
    if selected_action == "keep_open":
        return {
            ...
            "cohort_after": str(
                cast(dict[str, Any], row.get("action_cohort") or {}).get("id") or ""
            ),
        }
    if selected_action == "reclassify":
        ...
        return {
            ...
            "cohort_after": _feedback_action_cohort(new_action),
        }
...
def _feedback_decision_preview_item(
    *,
    decision: dict[str, Any],
    ...
    if not isinstance(action_cohort, dict):
        action_cohort = {}
    if not _feedback_status_is_open(source_item.get("status")):
        blockers.append(
            _feedback_decision_preview_blocker(
    ...
    return {
        ...
        "would_apply": _feedback_decision_would_apply(
            decision=decision,
...
def build_feedback_decision_preview_report(
    *,
    ...
    blockers: list[dict[str, str]] = []
    decision_payload, load_blockers = _load_feedback_decision_payload(
        resolved_decision_path
    )
    blockers.extend(load_blockers)
    decisions, decision_blockers = _feedback_decision_inputs(decision_payload)
    ...
    items = [
        _feedback_decision_preview_item(
            decision=decision,
    ...
    return {
        ...
        "mutation_boundary": _feedback_decision_preview_mutation_boundary(),
        "items": items,
        "blockers": all_blockers,
        "warnings": [],
    }
