...
class RunEvalSidecarStartTests(unittest.TestCase):
    ...
    def test_rejects_empty_repo_slug_before_state_derivation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                ...
                env={
                    **os.environ,
                    ...
                    "EVAL_SIDECAR_RUNTIME_ROOT": str(runtime_root),
                },
                capture_output=True,
                text=True,
            )
            ...
            self.assertEqual(result.returncode, 1)
            self.assertIn(
                "EVAL_SIDECAR_REPO_SLUG must be a non-empty repo slug",
                result.stderr,
            )
    ...
    def test_rejects_empty_min_seconds_before_start(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
