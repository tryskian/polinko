...
class CalibrateHallucinationThresholdTests(unittest.TestCase):
    ...
    def test_calibration_prefers_strictest_threshold_on_metric_tie(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            reports = root / "reports"
            reports.mkdir(parents=True)
            ...
            report_path.write_text(
                json.dumps(
                    {
                        "cases": [
                            {"id": "a", "pass": True, "score": 5, "risk": "low"},
                            {"id": "b", "pass": True, "score": 9, "risk": "low"},
                            {"id": "c", "pass": True, "score": 10, "risk": "low"},
                        ]
                    }
                ),
                encoding="utf-8",
            ...
            self.assertEqual(result.returncode, 0, msg=result.stderr)
            payload = json.loads(out_json.read_text(encoding="utf-8"))
            # Thresholds 0..5 all tie on metrics in this fixture; choose 5.
            self.assertEqual(payload["recommended_min_acceptable_score"], 5)
