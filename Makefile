...
PORTFOLIO_SANKEY_SOURCE_DB ?= .local/runtime_dbs/active/eval_viz.db
PORTFOLIO_SANKEY_OUTPUT_JSON ?= frontend/src/data/twin_sankey_raw.json
...
.PHONY: frontend-install frontend-build portfolio portfolio-open portfolio-sankey-data
...
frontend-build:
	@set -eu; \
	$(MAKE) --no-print-directory portfolio-sankey-data; \
	$(MAKE) --no-print-directory frontend-install; \
	npm --prefix "$(FRONTEND_DIR)" run build

portfolio-sankey-data:
	$(PYTHON) -m tools.export_portfolio_sankey_data \
		--source-db "$(PORTFOLIO_SANKEY_SOURCE_DB)" \
		--output-json "$(PORTFOLIO_SANKEY_OUTPUT_JSON)"
