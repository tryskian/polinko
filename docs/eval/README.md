...
- integrated manual-eval warehouse:
  ...
  - freshness source counts use the same evidence-bearing import scope as the
    rebuild command
  - read freshness with `make manual-evals-db-status`
  ...
  - health output breaks open feedback debt down by era/outcome and explicit
    OCR evidence relationships
  ...
  - materialise a local ignored feedback-decision draft with
  ...
  - preview a local human-reviewed feedback decision with
  ...
  - inspect overlay/OCR comparison readiness with
  ...
  - draft a local overlay/source image index with
  ...
  - validate the local overlay/source image index with
  ...
  - read the first OCR retry candidate packet with
  ...
  - verify selected OCR retry source candidates with
  ...
  - exact confirmation blockers use explicit source links
  - read source-history message provenance with
  ...
  - build a concrete OCR retry input packet with
  ...
  - build a concrete OCR retry source-artefact selection manifest with
  ...
  - rerun manifests expose source image names, OCR run IDs, resolution status,
    ...
    feedback closure
  - build a payload-only OCR retry rerun plan with
  ...
  - rerun plans expose stable source artefact IDs, feedback IDs, source
    ...
    `ARTIFACT_IDS=<artifact_id>` narrows the preview to a source artefact
  - build a human-selection OCR retry shortlist with
  ...
  - build a human-selection template with
  ...
  - template fingerprints stay in the draft payload for local stale-input
    review
  - validate local OCR retry human-selection JSON with
  ...
  - preview local OCR retry decision application with
  ...
  - apply previews require validation `state=ok` before emitting would-apply
    ...
    execution surface
  - check OCR retry execution readiness with
  ...
  - inspect local OCR retry execution bundles with
  ...
  - preview OCR retry feedback closure with
  ...
  - verify OCR retry feedback closure after apply with
  ...
  - preview OCR retry feedback-closure restore with
