<!-- @format -->

# Diagram Artefacts

This directory holds rendered SVG artefacts and the focused Mermaid source docs
that generate them.

## Source Docs

- [Eval Contract Diagrams](eval-contract.md)
- [Evidence And OCR Diagrams](evidence-and-ocr.md)
- [Refactor Method Diagrams](refactor-method.md)
- [Refactor Journey Diagrams](refactor-journey.md)

## Eval Contract SVGs

- [Polinko Eval Contract](polinko-eval-contract.svg)
- [Polinko Post-Fail Gate Stack](polinko-post-fail-gate-stack.svg)
- [Polinko Binary Eval Loop](polinko-binary-eval-loop.svg)

## Evidence And OCR SVGs

- [OCR Progress Funnel](ocr-progress-funnel.svg)
- [Current OCR Signal Shape](current-ocr-signal-shape.svg)
- [Operator Burden Signal Shape](operator-burden-signal-shape.svg)
- [Polinko Evidence Sankey](polinko-evidence-sankey.svg)
- [Beta Evidence Map](beta-evidence-map.svg)

## Refactor Method SVGs

- [Refactor Operating Loop](refactor-operating-loop.svg)
- [One Kernel Rule](one-kernel-rule.svg)
- [Refactor Source Hierarchy](refactor-source-hierarchy.svg)
- [Refactor Safety Gate](refactor-safety-gate.svg)
- [Clean Main Closeout](clean-main-closeout.svg)

## Refactor Journey SVGs

- [Refactor Journey: Evidence First](refactor-journey-evidence-first.svg)
- [Refactor Journey: Runtime Package Boundary](refactor-journey-runtime-package-boundary.svg)
- [Refactor Journey: Manual Eval Workbench](refactor-journey-manual-eval-workbench.svg)
- [Refactor Journey: Docs And Closeout](refactor-journey-docs-and-closeout.svg)
