...
- Polinko is entering the next method beta from a frozen Beta 2.3 snapshot:
  ...
  - `eval-sidecar` readiness attempt and sleep config uses defaults only when
    unset; explicit blank readiness config fails before launch
...
- closeout git helpers use defaults only when branch or remote config is
